...
public class MatchingTextHighlighter extends AbstractHighlighter {
     ...
     */
    public MatchingTextHighlighter(Painter<?> painter) {
    ...
    protected boolean canHighlight(Component component, ComponentAdapter adapter) {
        return painter != null && matchPattern != null;
    }
    ...
    protected boolean isLabelCompatible(Component component) {
        return component instanceof JLabel || (component instanceof WrappingIconPanel && ((WrappingIconPanel) component).getComponent() instanceof JLabel);
    }
    ...
    public void setMatchPatter(Pattern regex) {
    ...
    protected List<Rectangle> findHighlightAreas(JLabel object, int width, int height) {
        ...
        String clippedText = SwingUtilities.layoutCompoundLabel(object, fm, object.getText(), object.getIcon(), object.getVerticalAlignment(), object.getHorizontalAlignment(), object.getVerticalTextPosition(), object.getHorizontalTextPosition(), viewR, iconR, textR, object.getIconTextGap());
        ...
        return createHighlightAreas(object.getText(), object.getText(), fm, xOffset, yOffset, highlightHeight);
    }

    protected List<Rectangle> createHighlightAreas(String fullText,
            String clippedText, FontMetrics fm, int xOffset, int yOffset,
            int height) {
        List<Rectangle> areas = createHighlightAreas2( fullText, 
        ...
        if ( !fullText.equals( clippedText ) )
        {
            ...
            Matcher matcher = matchPattern.matcher( fullText );
            while ( matcher.find() )
                matchesInFullText++;
            ...
            matcher = matchPattern.matcher( clippedText );
            while ( matcher.find() )
     ...
     * 
     * @param highlightAreas
     *            a <code>List</code> of <code>Rectangle</code>s.
     */
    ...
    protected int calculateXOffset(JLabel component, Rectangle viewR, Rectangle textR, Rectangle iconR, int iconTextGap) {
        ...
        if (horizAlignment == SwingConstants.LEFT || (horizAlignment == SwingConstants.LEADING && leftToRight) || (horizAlignment == SwingConstants.TRAILING && !leftToRight)) {
            return textR.x;// respect the icon and start the highlight at the
            // beginning of the text not at 0
        } else if (horizAlignment == SwingConstants.RIGHT || (horizAlignment == SwingConstants.TRAILING && leftToRight) || (horizAlignment == SwingConstants.LEADING && !leftToRight)) {
            int offsetWhenRight;
            if (leftToRight)
                offsetWhenRight = viewR.width - textR.width;
            else {
                int currentIconTextGap = component.getIcon() != null ? iconTextGap : 0;// The
                                                                                       // gap
                                                                                       // between
                                                                                       // the
                                                                                       // icon
    ...
    private class DelegatingPainter implements Painter<JLabel> {
        ...
        public void paint(Graphics2D g, JLabel object, int width, int height) {
            ...
            for (Rectangle r : highlightAreas) {
                ...
                ((Painter)painter).paint(scratchGraphics, object, r.width, r.height);
